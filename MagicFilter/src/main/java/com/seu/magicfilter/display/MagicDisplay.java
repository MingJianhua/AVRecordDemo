...
public abstract class MagicDisplay implements Renderer{
	...
	protected int mCameraTextureId = OpenGLUtils.NO_TEXTURE;
	...
	public void setFilter(final int filterType) {
		Log.v("MegicDisplay", "SetFilterType = "+filterType);
	...
	protected void getBitmapFromGL(final Bitmap bitmap,final boolean newTexture){
		mGLSurfaceView.queueEvent(new Runnable() {
			...
			public void run() {
            	...
            	long mReadStamp = System.currentTimeMillis();
            	
            	if( mCurrentStamp == 0 ){
        			mCurrentStamp = mReadStamp;
        		}
        		
        		framerate ++;
        		
        		if( mReadStamp - mCurrentStamp >= 1000 &&  mCurrentStamp != 0 ){
        			Log.i( "***" , "framerate2==>" + framerate  );
        			framerate = 0;
        			mCurrentStamp = mReadStamp;
        		}
                ...
                Log.i( "" , "consume2 == >  " + ( afterRead -  mReadStamp ) );
